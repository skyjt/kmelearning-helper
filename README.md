...
[![版本](https://img.shields.io/badge/版本-v2.12.0-1f6feb)](https://github.com/skyjt/kmelearning-helper/releases/latest)
...
> **为什么默认不倍速、不跳过？** 平台判断一门课是否完成，不只看视频有没有播到结尾，而是综合心跳、真实学习时长、播放进度和完成状态。第三方倍速插件就算把视频播完，也可能因真实时长不够而不算完成；拖进度条、跳过播放通常也过不了完成判断。本扩展锁定 1x；结束后先等待平台确认并刷新学习记录，确认时长仍有缺口才继续补学。
...
| --- | --- |
| **自动播放** | 进入课程后自动开始播放视频 |
| **未完成自动补学** | 视频播完后先等待并刷新学习记录；确认时长不足时才按 1x 补学 |
...
最近一次更新（**v2.12.0**）：视频结束后会等待平台确认并按目录实际时长刷新学习记录，已完整学习时不再从头重播；同时修复浮窗元素显示、目录页路径识别、补学重试空转等问题，并优化小窗口滚动、状态展示、悬浮球误触和后台扫描。

| 版本 | 主要变化 |
| --- | --- |
| **v2.12.0** | 修复浮窗多处显示失效与自动学习若干边界问题；浮窗正面限高、状态截断、点击展开、Esc 取消；后台标签页与答题区扫描性能优化 |
